...
class StarGeometry extends THREE.BufferGeometry {
    ...
    static GEOMETRY_TYPE_DEFAULT = 0;
    static GEOMETRY_TYPE_SPIRAL = 1;
    
    constructor( points, type = StarGeometry.GEOMETRY_TYPE_DEFAULT, color = null) {
	super();
	...
	const arms = 3;
	const points_per_arm = points/arms;
	for( var i=0; i<points; i++ )
	{
	    if (type == StarGeometry.GEOMETRY_TYPE_DEFAULT){
		...
		const arm_pos = (i % points_per_arm)/points_per_arm;
		const arm = Math.floor(i/points_per_arm);
		v.x = (3**(2*arm_pos)-1)*Math.cos(4*Math.PI*arm_pos+2*arm*Math.PI/arms)+2*(Math.random()**3-0.5)*arm_pos**2;
		v.y = (3**(2*arm_pos)-1)*Math.sin(4*Math.PI*arm_pos+2*arm*Math.PI/arms)+2*(Math.random()**3-0.5)*arm_pos**2;
		v.z = 2*(Math.random()-0.5)*arm_pos;
	    }
